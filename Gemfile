source 'https://rubygems.org'
ruby '2.3.1'

gem 'bundler'

# replace SecureRandom
gem 'sysrandom', '>= 1.0.3', '~> 1.0.3', require: 'sysrandom/securerandom'
gem 'rbnacl-libsodium', '~> 1.0'
gem 'rbnacl', '~> 3.4'
gem 'json_web_token', '~> 0.3.2'
gem 'json', '~> 2.0'
gem 'puma', '~> 3.5'
gem 'rack', '~> 1.6'
gem 'rack-contrib', '~> 1.4', require: 'rack/contrib'
gem 'rack-attack', '~> 5.0', require: 'rack/attack'
gem 'rack-robustness', '~> 1.1', require: 'rack/robustness'
gem 'rack-cors', '~> 0.4.0', require: 'rack/cors'
gem 'rack-json_web_token_auth', '~> 0.2.0', require: 'rack/json_web_token_auth'
gem 'rack-content_security_policy', '~> 0.1.0', git: 'https://github.com/grempe/rack-content_security_policy.git', require: 'rack/content_security_policy'
gem 'rack-bounce', '~> 0.1', require: 'rack/bounce'
gem 'sinatra', '~> 1.4', require: 'sinatra/base'
gem 'sinatra-param', '~> 1.4', require: 'sinatra/param'
gem 'dotenv', '~> 2.1'
gem 'activesupport', '~> 5.0'
gem 'redis', '~> 3.3'
gem 'redis-namespace', '~> 1.5'
gem 'redis-activesupport', '~> 5.0'
gem 'sidekiq', '~> 4.2'
gem 'sidekiq-scheduler', '~> 2.0'
gem 'jsender', '~> 0.2'
gem 'tierion', '~> 1.3'
gem 'rollbar', '~> 2.12'
gem 'vault', '~> 0.7'
gem 'session_keys', '1.0.0'
gem 'rethinkdb', '~> 2.3'
gem 'sirp', '~> 2.0.0'
gem 'tss', '~> 0.4.2'
gem 'uuidtools', '~> 2.1.5'
gem 'foreman', '~> 0.82'

group :test, :development do
  gem 'rake'
  gem 'rspec'
  gem 'rack-test'
  gem 'pry'
  gem 'pry-byebug'
  gem 'guard-puma', require: false
  gem 'guard-rspec', require: false
  gem 'wwtd'
  gem 'mock_redis', '~> 0.17'
  gem 'fasterer'
  gem 'faker'
  gem 'rubocop'
end
