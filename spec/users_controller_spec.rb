...
RSpec.describe UsersController do
  ...
  let(:password) { Faker::Internet.password(32) }
  let(:keys)     { SessionKeys.generate(username, password) }
  let(:auth)     { SIRP::Verifier.new(4096).generate_userauth(keys[:id], keys[:hex_keys][0]) }
